...
\title{Automatic Game Parameter Tuning and Playtesting via Active Learning}
...
The weights of all models are averaged and the point with lowest probability is chosen. \mytodo{find citation}\todo[inline]{find citation}
...
\section{Experiments}
Our experiments sought to test whether the active learning framework could reduce the number of human playtesters needed to tune design parameters.
In the experiments we used the active learning acquisition functions given above for tuning enemy parameters or tuning player controls.
...
criteria: min 10 waves total, only keep waves 1-10 for comparability

same test process as before but now using all data gathered as pool to sample from
...
overview results for fitting each model
emphasis on AL, specific methods varied widely and likely to vary in other applications as well
...
\caption{regression simulation; mse; down is good}
\label{fig:reg_sim}
\end{figure}
...
\caption{regression experiment; mse; down is good}
\label{fig:reg_expr}
\end{figure}
...
\caption{classification simulation; f1; up is good}
\label{fig:cls_sim}
\end{figure}
...
\caption{classification experiment; f1; up is good}
\label{fig:cls_expr}
\end{figure}
    GP random beats KSVM random -> GP is better able to model domain
