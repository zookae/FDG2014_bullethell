...
%, automatically adjust model complexity to data, and \mytodo{integrate out} \todo{don't understand} parameters without user intervention \cite{rasmussen2006}. %%% MOR: THIS FELT REDUNDANT WITH BELOW.
Intuitively, non-parametric models allow an infinite number of variables to account for the data before selecting only the subset needed to explain a given set of observations. In practice, this leads to models that automatically become more complex to fit a data set as needed. 
...
\bibliography{lib}  % sigproc.bib is the name of the Bibliography in this case
